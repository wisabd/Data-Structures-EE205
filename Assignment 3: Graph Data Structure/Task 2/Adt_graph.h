...
class ADT_Node {
	public:
		...
		ADT_Node<T> *prev;
		int set_node_value(T newValue);
		int set_prev(ADT_Node<T>* previous);
		int set_D_value(int newValue);
		int set_previous(int p);
		int D_value;
		int previous;
	...
	//private:
		...
		/* MODIFIED PART, FOR TASK 2 ONLY */
		/* Due to this modification, constructor needs to be chagned as well */
...
class ADT_Graph {
	public:
		int vertices(void);
		int edges(void);
		int insertVertex(string node_name, int node_value);
		int insertEdge(string vertex_name1, string vertex_name2, string edge_name, int edge_value);
